...
- ✅ **支持级联**（阳哥 2026-08-30 确认）：
  **每台取 2 路输出接到下一台 ADS48 的输入**，与 IPS UM2000ATD 同一套规则。
  级联后每台（非末台）可用出口 = `8 − 2 = 6` → 带 **3 台**接收机；末台用满 8 出 → 带 4 台。
...
| 首台（末台）可带接收机 | `8 ÷ 2 = 4` 台 |
| 非末台可带接收机 | `(8 − 2) ÷ 2 = 3` 台 |
| 天线对数 | 整套系统 **1 对**（2 支）接首台进口，后续台由级联供信号 |

| 接收机数 | 1–4 | 5–7 | 8–10 | 11–13 |
|---|---|---|---|---|
| 所需 ADS48 | 1 | 2 | 3 | 4 |

对照 IPS（UM2000ATD：10 出、每机 4 口、级联留 2 口）：
首台带 2 台，之后每台 +2 台。AUDIX 台均带机数更高，因为每台接收机只占 2 口而非 4 口。
...
- `params.cascade_outs`：IPS UM2000ATD = **2**；AUDIX ADS48 = **2**
  （两者都能级联，规则一致：每台取 2 出口接下一台进口）
...
1. ~~ADS48 是否支持级联~~ → **已确认支持**（阳哥 2026-08-30），`cascade_outs=2` 已落库。
